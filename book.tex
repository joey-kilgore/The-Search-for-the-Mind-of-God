...
Modeling is how we conduct science. This doesn't always mean that our models are run as simulations (though that may be the common use), but they are much more than that. We can go back to 2002 when the MacIntyre Richardson Grill (MRG) model of the myelinated axon was created. That set of equations used in conjunction with implicit differentiation methods allowed for modeling the electrical impulses of an axon. Going back further, we could go to 1952 when Hodgkin and Huxley created their famous HH model (also named after themselves) which modeled unmyelinated giant squid axons. But certainly models go much further back! All the way back in the 17th century we can look at how Galileo modeled gravity. Going back further still, we could see how, in around 400BC, the models of the universe were created. Going back to the first known mathematical theory we can see how the Babylonians and other civilizations were creating models of triangles (with what are now known as Pythagorean triples). All of this gives us a rich understanding that the entire history of science has been about creating models. Those could be mathematical models (like gravity or Pythagorean triples), those could also be mathematical models used specifically in a computational setting (like the MRG model), or they could even be biological models (like the sprague dawley rats used in every one of our \textit{in-vivo} experiments). 
